...
class Line {
    ...
    public void init() {
	this.leading = 0;
	while (this.leading < this.value.length()
    ...
    private boolean checkImage(final String in, int start) {
	...
	if (this.value.length() - this.leading >= 2
		&& this.value.charAt(this.leading + 1) == ' ') {
	...
	if (this.value.length() - this.leading >= 3
	    ...
	    while (i < this.value.length()
		    && Character.isDigit(this.value.charAt(i)))
		i++;
	    if (i + 1 < this.value.length() && this.value.charAt(i) == '.'
	...
	if (this.next != null && !this.next.isEmpty) {
	    if ((this.next.value.charAt(0) == '-')
		    && (this.next.countChars('-') > 0))
	...
	if (start + 3 < line.value.length()) {
		...
		int pos = start + 4;
		while (line != null) {
		    while (pos < line.value.length()
			    && line.value.charAt(pos) != '-') {
			pos++;
		    }
			...
			if (pos + 2 < line.value.length()) {
			    if (line.value.charAt(pos + 1) == '-'
				    && line.value.charAt(pos + 2) == '>') {
